...
if [[ $? != 0 ]]; then
    ...
    npm install -g react-native-cli 2> /dev/null
    echo Installed
fi
...
if [[ $? != 0 ]]; then
    ...
    brew upgrade cocoapods &> /dev/null
fi
...
if [[ $? != 0 ]]; then
    ...
    echo Check to see that the repo exists and there isn\'t already a file/directory named treasurehunt
    exit $?
fi
...
rm -rf ./treasurehunt.xcworkspace
pod install
cd ..
